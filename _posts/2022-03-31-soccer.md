...
Figure 2 shows that the number of games per year has been increasing, with a peak in 2018 of over 300 games. It also shows that, out of the 8 world cups that have been played so far, the United States have won half of them.

The US have been dominant in women’s football, so it’s no surprise they’re ranked number 1 in the world. So, to take a closer look at how the US compares with other top teams, I created Figure 9 to show the proportion of wins, draws, and losses for the [top 4 ranked teams](https://www.fifa.com/fifa-world-ranking/women?dateId=ranking_20220325).

Using functions from dplyr and ggplot2, I filtered the data by country and created the stacked column graph showing the proportions with both colour and text labels.
...
![figure 2](/img/posts/soccer/plot02.png)
<span class="caption text-muted">Figure 2. Game result proportion breakdown for the top 4 teams</span>

Figure 9 shows that the win_proportion (green) of the US is the largest, followed by Germany, Sweden, then France. And their lose_proportion (red) is the smallest compared to the others.

#### Conclusion

With all the different packages available in R, there are so many options for visualisations. Using packages like plotly and shiny you can even create interactive and animated plots.

Another great opportunity is how, when you get stuck on something while using R, you can almost always find the answer to your question online. There are so many resources available to help you through your analysis, which I think is a great opportunity.

On the other hand, with so many resources available, it can be difficult to find the right answer to your question quickly. So this can be a challenge, as it can take up time.

Despite having a steep learning curve when starting with R, I think its opportunities significantly outweigh the challenges. So I think R is an excellent tool, and I recommend it to others.